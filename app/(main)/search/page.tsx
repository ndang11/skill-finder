					...
					<div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10 py-8 sm:py-12">
						...
						<div className="max-w-4xl mx-auto space-y-3">
							...
							<div className="flex items-center justify-center gap-2 text-xs font-semibold text-gray-600">
								...
								>
									<Search className="h-3 w-3" /> {t("searchPage.searchModeAll")}
								</button>
								<button
									type="button"
								...
								>
									<Zap className="h-3 w-3" /> {t("searchPage.searchModeSkill")}
								</button>
								<button
									type="button"
								...
								<div className="mt-4 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm space-y-5 text-left animate-in fade-in slide-in-from-top-2 duration-200">
									...
									<div>
										...
										<div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto pr-1">
											...
											{CAMEROON_SKILLS.map((skill) => {
												const IconComponent = skill.icon || MoreHorizontal;
												const displayName =
										...
										<div className="flex flex-wrap gap-2">
											{[4, 3, 2, 1].map((rating) => (
												...
												>
													...
													{rating}+ {t("searchPage.ratingOrAbove")}
												</button>
											))}
										</div>
									</div>
					...
					{!loading && (
						<div className="mb-6 flex items-center justify-between">
							<p className="text-sm font-semibold text-gray-600">
								{filteredProfessionals.length} {t("searchPage.resultsFound")}
	...
	return (
		<Suspense
			fallback={
				<div className="min-h-screen bg-gray-50/50 flex flex-col justify-between">
					<Header />
					<div className="flex-1 mx-auto max-w-7xl px-4 sm:px-6 lg:px-10 py-8 sm:py-10">
						<ProfessionalCardSkeletonGrid />
					</div>
					<Footer />
				</div>
